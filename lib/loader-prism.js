...
exports.prism = ({ name, value }) => {
  if(!PRISM_LANGUAGES.includes(name))
  ...
  const highlighted = Prism.highlight(value, Prism.languages[name]);
  return `<pre class="language-${name}"><span class="extension">${name}</span><code class="language-${name}">${highlighted}</code></pre>`;
};
